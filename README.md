...
4. 試合後に管理者が `🇦 / 🇧` または GUI の `A勝ち / B勝ち` で結果を記録する
...
| `/split prev` | 過去 5 試合分の履歴から後追いで勝敗記録する（管理者専用） |

`/split run` の結果メッセージには以下のリアクションを付けられます。

| リアクション | 説明 |
|---|---|
| `🇦` | Team A 勝利として記録（管理者のみ） |
| `🇧` | Team B 勝利として記録（管理者のみ） |
...
- `🇦 / 🇧` はそのメッセージに紐づいたチーム構成を使って記録します。記録できるのは管理者のみです。
...
| コマンド | 説明 |
|---|---|
| `/result <team>` | `last_match` を使って手動で勝敗記録する（管理者専用） |
| `/result_undo` | 直前の勝敗記録を取り消す（管理者専用） |
| `/stats show [member] [period]` | 通算または当日の戦績を表示する |
| `/stats reset` | 戦績を全削除する（管理者専用） |
| `/stats export` | 戦績 JSON をエクスポートする（管理者専用） |
| `/stats import <file>` | 戦績 JSON をインポートしてマージする（管理者専用、1MBまで） |
...
| `/config role_balance <atk> <all> <spd> <deff> <sup>` | `b=2` 用のロール構成を設定する（管理者専用） |
| `/config avoid <count>` | 連続ロール回避回数を設定する（管理者専用） |
| `/config vc <team> <channel>` | デフォルト VC を設定する（管理者専用） |
| `/config start_announce <minutes>` | VC 移動後のスタート告知時刻を設定する（管理者専用） |
| `/config show` | 現在の設定を表示する |
| `/config reset` | チーム分けコード、ロール構成、連続ロール回避設定をリセットする（管理者専用） |
...
保存先を変えたい場合は `UNITECHAN_DATA_DIR` にディレクトリを指定できます。
